...
<!-- I question the location of this and perhaps even whether it's needed? In any case, setup should come first so people can work on that in the background, while I go over introductory stuff -->

* `R/` for your code
* `man/` for function documentation
* `tests/` for tests
* `DESCRIPTION`, the metadata
* `NAMESPACE`, what goes in and what comes out

## Five forms of a package

<!-- Same concern about location and content. If I talk about this, I usually use a figure from R Packages to make if more evocative, not just text. -->

* **Source**: a directory of files. What you work on
* **Bundle**: compressed to one `.tar.gz`, via `R CMD build`
* **Binary**: platform-specific `.tgz` or `.zip`
* **Installed**: decompressed into a library, via `install.packages()`
* **In memory**: loaded and attached, via `library()`
...
<!-- I expected to see use_github() right around here. -->
...
<!-- Use strikethrough on the `source()` and "send the function" lines to discourage actual usage. -->

## `load_all()`

<!-- Needs the figure in Andy's slides. Ask me for help if you can't find this. -->
...
<!-- Not sure why we waited til now for use_github(). -->
...
* `use_mit_license()`
* `check()` again, commit, and `use_github()`
...
<!-- Use the screenshot I have in the module for this. -->
...
<!-- `document()` should get a bullet. Make parallel to other bullet lists of package dev moves. -->
...
<!-- Needs the figure Andy uses for this workflow. -->

<!-- Andy also has a slide 30 with joint workflow for editing code and edit documentation. -->
...
<!-- Use the screenshot from the module. -->
...
<!-- Andy's slides 38, 39, 40 have good workflow figures. We need those. -->
...
<!-- Andy's slide 49 has a workflow figure with all 3 of edit code, edit tests, edit roxygen. We need to include that. -->
...
<!-- Move "fine print" content to next slide here. -->
...
The release is then a small step, not a cliff.
...
* Edit code, then `load_all()`
* Edit tests, then `test()`
* Edit roxygen, then `document()` and `?fun`
* Periodically `check()`
* Commit and push
...
<!-- I like this table but the last row appears to be cutoff? Feels like the font size needs to autoscale for the space available. -->
