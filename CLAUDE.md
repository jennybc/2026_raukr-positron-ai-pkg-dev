...
* **In progress: the Positron and Posit Assistant session.** `positron/index.qmd` is an annotated inventory of the existing material, written so that it stands on its own as a useful set of pointers for learners. The open design question is how much to inline here versus link out to the Positron workshop site; see "Sources for the Positron material" below.
  * **Scope, decided 2026-07-31: a thorough introduction to Positron, plus awareness of Posit Assistant.** Not a full AI module. Other RaukR sessions later in the week cover more AI ground, including one on R + LLMs, and a colleague who is a Quarto expert teaches two half-day sessions. So agent modes, skills, and prompt craft are out of scope, and the **Quarto module can be cut hard or dropped**.
  * **Getting signed in to a model provider is a featured part of the session** and the thing students cannot get elsewhere. Assistant coverage otherwise is roughly 20-25 minutes: sign-in, what it is, one demo, where to learn more.
  * **Assistant content is demo-driven, not follow-along.** Jenny drives the demo; materials get published for students adept enough to follow, everyone else watches. This deliberately avoids needing the whole room authenticated in lockstep.
  * **This is the first session of RaukR, so it is the de facto setup session.** No pre-work is assigned. Assume some students install Positron while Jenny is talking and configure a provider later, at lunch or in the evening. **Watching must be a first-class mode; nothing may be gated on working software.** Open the session with the download link before any slides, and put hands-on work as late as the narrative allows.
  * **Working plan lives in `notes-private/workplan.md`**: seven 30-minute chunks between 2026-08-01 and the workshop, each with a "done means" test. `notes-private/README.md` has a start-here index. Site mechanics are already scaffolded; what remains is Keynote work, one live verification pass, the new Assistant deck, and a timed run-through.
...
- `_quarto.yml`: website config; theme is `[zephyr, footer.scss]`. Output dir is the Quarto default `_site/`, gitignored.
  ...
  - No `resources:` key, and none is needed to publish static files: **Quarto copies non-source files from the project directory into `_site/` automatically.** Verified 2026-07-31, `_site/pkg-dev-2023.pdf` exists, so Andy's 2023 deck *is* in fact published, contrary to what this file used to claim. If something should stay unpublished, it needs an actual exclusion, not just the absence of `resources:`.
  - Navigation is a sidebar, not a navbar: Home, a divider, then the two sessions in chronological order.
    - Package development is a `section:` carrying its own `href:` to the landing page, with modules listed **explicitly** under `contents:`. Add each new module by hand.
    - **Positron is deliberately a plain `href:` even though it has module pages.** Its landing page already carries a module table, and duplicating that in the sidebar felt redundant. Consequence to accept: the deck pages do not get sidebar entries or prev/next page navigation, so the landing page table is the only way to move between them. That is fine because they are thin frames around PDFs, not reading material. Do not "fix" this by adding a section.
...
- **The Positron session frames PDF decks, it does not use revealjs.** Each `positron/NN-*.qmd` is a title, a link to `slides/NN-*.pdf`, and a raw-HTML iframe of the same file. This copies the proven pattern from the `positron-workshop` site, where the Keynote-built decks are exported to PDF and the pages are thin frames around them. Deliberately not using that site's `_slides.md` include, since it depends on a `{{< slug >}}` shortcode; the explicit filename per page is simpler.
  - **To update a deck, overwrite `positron/slides/NN-name.pdf`. Nothing else changes.**
  - The Keynote sources live in **`_keynote/`**, gitignored. The leading underscore is load-bearing: Quarto skips `_`-prefixed directories, so ~121 MB of `.key` files never get copied into `_site/`. Copied from `~/work/2025_positron-tutorial/slides/2025-08-08_useR-as-delivered/`, which stays untouched as the archive.
  - Module numbering mostly keeps the 2025 names, with two deliberate changes. **`04-quarto` is intentionally absent**, since a colleague teaches Quarto later in the RaukR program. And the 2025 `07-beyond` has been **split**: `07-posit-assistant` is a new deck Jenny still has to build (the Assistant intro plus the sign-in walkthrough), and `08-beyond` is the old closer, reduced to the Python and RStudio "elephants" now that AI has its own module. Assistant comes before the closer, matching the running order in `notes-private/workplan.md`.
  - The PDFs currently in `positron/slides/` are **placeholders**: single-page, ~50 KB each, generated from the first slide of the 2025 useR decks purely to prove the wiring. The real exports are ~50 MB in total, 32 MB of which is `01-hello-positron` alone, so watch repo weight when the real ones land and consider a lower-quality Keynote export. A callout on `positron/index.qmd` announces the placeholder state and should be deleted once real decks are up.
...
**Ryan Johnson's Positron + Posit Assistant workshop.** Slides: <https://docs.google.com/presentation/d/1GT1W22iAjQYTNRYU_ML9_y1rTPxKSZOtVoCF7iXzpTI/edit>. The slides are sparse; the substance is in the delivery. **Transcript of a 2026-07-15 recording is in `notes-private/`, digested into `notes-private/ryan-demo-notes.md`.** This turns out to be the single most useful source for the Positron session: a 90-minute workshop covering Positron basics *and* Assistant with two hands-on exercises, delivered multiple times a month, and more current than the Positron docs. Read those notes before planning the session. Highlights worth stealing: a deliberately **silent ggplot bug** as the Assistant motivating example, the "creepy eyeballs" framing for environment context, the per-tool approval model, and the "WHERE loop" for AI-assisted EDA.

**Decided: RaukR is not using Instruct**, the sandbox tool that makes Ryan's logistics effortless. Jenny and Ryan concluded the hassle and cost do not pay off for a two-week workshop built around project work; students need to stand up their own systems and keep them working. Consequence: **getting students authenticated to a model provider is a real, unavoidable problem and the top open question for this session.** It is pre-workshop homework, so the answer belongs in the "Before the workshop" section of `positron/index.qmd`. Candidate paths and the GitHub Student Developer Pack angle are in `notes-private/ryan-demo-notes.md`. Do not propose Instruct again.
