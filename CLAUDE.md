...
* **Package development content is done.** `pkg-dev/index.qmd` plus modules `01-setup.qmd` through `08-share.qmd`, and the slide deck `slides/pkg-dev.qmd`. Adapted from [Andy Teucher's posit::conf(2023) workshop](https://github.com/posit-conf-2023/pkg-dev), rewritten for Positron, built around the libminer example.
  ...
  * The deck is ready. A few optional `<!-- TODO -->` comments in it mark her open questions; leave them for her.
* **In progress: the Positron and Posit Assistant session.** `positron/index.qmd` is a landing page and index into a collection of module-specific slide decks. Decks 01, 02, 03, 05, 06 are done. `07-posit-assistant` is all that remains: getting signed in to a model provider, plus a small usage demo.
...
- **The Positron session frames PDF decks, it does not use revealjs.** Each `positron/NN-*.qmd` is a title, a link to `slides/NN-*.pdf`, and a raw-HTML iframe of the same file. This copies the proven pattern from the `positron-workshop` site, where the Keynote-built decks are exported to PDF and the pages are thin frames around them. Deliberately not using that site's `_slides.md` include, since it depends on a `{{< slug >}}` shortcode; the explicit filename per page is simpler.
  ...
  - Module numbering keeps the 2025 names, minus two decks. **`04-quarto` is intentionally absent**, since a colleague teaches Quarto later in the RaukR program. And **there is no closer deck**: the 2025 `07-beyond` became `07-posit-assistant`, so `08` does not exist.
  - Watch repo weight on the PDF exports. The 2025 originals were ~50 MB in total, 32 MB of which was `01-hello-positron` alone; a lower-quality Keynote export keeps them reasonable.
  - `07-posit-assistant.pdf` is still a placeholder, and a callout on `positron/index.qmd` says so. Delete the callout once the real deck is up.
...
**Jenny's RaukR 2025 Keynote file**, `~/work/2025_raukr-pkg-dev/pkg-dev-whole-game-raukr.key`. The single best source: her own prior version of this exact workshop, same audience, same half-day format. All current figures came from here.
...
* **Assistant coverage exists, but only in the decks**, so grepping the `.qmd` files for assistant, copilot, or AI misleadingly returns nothing. `07-beyond` (useR version) gives about 5 of its 17 slides to Positron Assistant, as one of three "elephants in the room" alongside Python and the future of RStudio. Do not count it as reusable: it is a 5 to 7 minute FAQ answer about a product being deprecated, so the Assistant substance is a rebuild. Details in `notes-private/deck-07-summary.md`.
